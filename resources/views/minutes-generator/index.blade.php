...
<body>
    <main class="page">
        <div class="topbar">
            ...
            <form method="POST" action="{{ route('minutes-generator.logout') }}">
                @csrf
        ...
        <section class="stats" aria-label="Indicadores">
            <article class="stat"><span>Total</span><strong>{{ $stats['total'] }}</strong></article>
            <article class="stat"><span>Borradores</span><strong>{{ $stats['draft'] }}</strong></article>
            <article class="stat"><span>Aprobadas</span><strong>{{ $stats['approved'] }}</strong></article>
            <article class="stat"><span>Mayor versión</span><strong>{{ $stats['versions'] }}</strong></article>
        </section>
        ...
        <div class="grid">
            <section class="card">
                <h2>Nueva minuta</h2>
                <form id="analysis-form" method="POST" action="{{ route('minutes-generator.analyze') }}">
                    @csrf
                    <label for="transcript_text">Transcripción</label>
                    <textarea id="transcript_text" name="transcript_text" required maxlength="{{ $maxTranscriptLength }}" placeholder="Pegá acá la transcripción completa de la reunión...">{{ old('transcript_text') }}</textarea>
                    <div class="actions">
                        <div>
                            <p class="hint">Mínimo 100 caracteres. La IA tratará la transcripción como datos, no como instrucciones.</p>
                            <p id="counter" class="counter">0 / {{ number_format($maxTranscriptLength) }} caracteres</p>
                        </div>
            ...
            <section class="card">
                ...
                <form class="filters" method="GET" action="{{ route('minutes-generator.index') }}">
                    <input name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Buscar por título, resumen o fecha">
                    <select name="status">
                            ...
                            @foreach ($minutes as $minute)
                                <tr>
                                    <td>
                                        ...
                                        <small>{{ $minute->meeting_date }} · {{ $minute->created_at->format('Y-m-d H:i') }}</small>
                                    </td>
                                    <td><span class="status {{ $minute->status }}">{{ $minute->status }}</span></td>
                                    <td>{{ $minute->version }}</td>
                                    <td><a href="{{ route('minutes-generator.show', $minute) }}">Ver</a></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                    <p class="pagination">Mostrando {{ $minutes->firstItem() }}-{{ $minutes->lastItem() }} de {{ $minutes->total() }} minutas.</p>
                    @if ($minutes->hasPages())
                        <nav class="pager" aria-label="Paginación de minutas">
                            ...
                            @else
                                <a class="pager-control" href="{{ $minutes->previousPageUrl() }}" rel="prev">Anterior</a>
                            @endif

                            <span class="pager-current" aria-current="page">Página {{ $minutes->currentPage() }} de {{ $minutes->lastPage() }}</span>

                            @if ($minutes->hasMorePages())
                                <a class="pager-control" href="{{ $minutes->nextPageUrl() }}" rel="next">Siguiente</a>
                            @else
